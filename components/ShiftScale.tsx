...
// --- Sub-components (Internal to ShiftScale for better encapsulation) ---
...
// --- Main Scale Component ---

interface ShiftScaleProps {
    shiftType: 'day' | 'night';
    title: string;
    intervals: string[];
}

const ShiftScale: React.FC<ShiftScaleProps> = ({ shiftType, title, intervals }) => {
    const { dayPosts, nightPosts, addPost, updatePost, deletePost, user } = useApp();
    const [isLocked, setIsLocked] = useState(false);

    const posts = shiftType === 'day' ? dayPosts : nightPosts;
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            if (post && post.group !== groupId) {
                updatePost(shiftType, post.id, { group: groupId });
            }
        }
    };
    ...
    const handleAddClick = (group: GroupType) => {
        setModalMode('add');
        setFormData({ name: '', officer: '', group: group, equipment: [], status: 'Ativo' });
        setIsModalOpen(true);
    };
    ...
    const handleSave = () => {
        ...
        if (modalMode === 'add') {
            addPost(shiftType, postData);
        } else if (selectedPostId) {
            updatePost(shiftType, selectedPostId, postData);
        }
    ...
    const toggleEquipment = (eq: string) => {
        setFormData(prev => {
            ...
            return {
                ...prev,
                equipment: exists ? prev.equipment.filter(e => e !== eq) : [...prev.equipment, eq]
            };
        });
    };
                    ...
                    <div className="flex flex-col border-2 border-slate-900 dark:border-slate-500 bg-white dark:bg-[#111318] shadow-2xl">
                        {GROUPS.map(group => (
                            <DroppableGroup
                                key={group.id}
                                group={group}
                                posts={posts.filter(p => p.group === group.id)}
                                onAdd={handleAddClick}
                                onEdit={handleEditClick}
                                onDelete={(id) => deletePost(shiftType, id)}
